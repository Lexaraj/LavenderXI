...
m:addOverride("xi.actions.spells.trust.apururu_uc.onMobSpawn", function(mob)
    ...
    if lvl >= 30 then	
		mob:addGambit(ai.t.PARTY, { ai.c.HPP_LT, 70 }, { ai.r.MA, ai.s.SPECIFIC, xi.magic.spell.CURE_III })
	end

    if lvl >= 17 then	
		mob:addGambit(ai.t.PARTY, { ai.c.HPP_LT, 75 }, { ai.r.MA, ai.s.SPECIFIC, xi.magic.spell.CURE_II })
	end

    if lvl >= 1 then	
		mob:addGambit(ai.t.PARTY, { ai.c.HPP_LT, 75 }, { ai.r.MA, ai.s.SPECIFIC, xi.magic.spell.CURE })
	end
